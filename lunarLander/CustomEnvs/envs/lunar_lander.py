...
VIEWPORT_W = 600
VIEWPORT_H = 900 # Increase the height from 400 to 900
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def reset(
        ...
        *,
        ...
        # Random chunk for the helipad
        random_chunk = self.np_random.integers(1, CHUNKS - 1)
        self.helipad_x1 = chunk_x[random_chunk - 1]
        self.helipad_x2 = chunk_x[random_chunk + 1]
        self.helipad_y = H / 8
        height[random_chunk - 2] = self.helipad_y
        height[random_chunk - 1] = self.helipad_y
        height[random_chunk + 0] = self.helipad_y
        height[random_chunk + 1] = self.helipad_y
        height[random_chunk + 2] = self.helipad_y
        smooth_y = [
        ...
        # Create the object to be used to discretize the space for the shields
        self.terrain_info = {
            "chunk_x": chunk_x,
            "smooth_y": smooth_y,
            "helipad_x1": self.helipad_x1,
            "helipad_x2": self.helipad_x2,
        ...
        # self.lander.ApplyTorque(self.np_random.uniform(-20, 20), True)
    ...
    def step(self, action):
        ...
        Y_weight = (VIEWPORT_H / 400.0) 
        alpha = 10.0          # tuning coefficient: larger → stronger incentive to slow down
        shaping = (
            -100 * state[1] * Y_weight  # To force it to try to land on the ground, but remove the dependance on the helipad location
            - 100 * np.sqrt(state[2] * state[2] + state[3] * state[3])
            - 100 * abs(state[4])
            + 10 * state[6]
            + 10 * state[7]
            - alpha * np.sqrt(state[3] ** 2)
        ...
        for obj in self.particles + self.drawlist:
            for f in obj.fixtures:
                ...
                for x in [self.helipad_x1, self.helipad_x2]:
                    ...
                    flagy2 = flagy1 + 50
                    sign = np.sign(x - (self.helipad_x1 * SCALE)) # used to mirror the flags
                    sign_pos_neg = -1 if sign <= 0 else 1
                    pole_x = x + (-2 * sign_pos_neg)
                    pygame.draw.line(
                        self.surf,
                        color=(255, 255, 255),
                        start_pos=(pole_x, flagy1),
                        end_pos=(pole_x, flagy2),
                        width=1,
                    )
                    pygame.draw.polygon(
                        self.surf,
                        ...
                        points=[
                            (pole_x , flagy2),
                            (pole_x, flagy2 - 10),
                            (pole_x + (-25 * sign_pos_neg), flagy2 - 5),
                        ],
                    )
                    gfxdraw.aapolygon(
                        self.surf,
                        [(pole_x, flagy2), (pole_x, flagy2 - 10), (pole_x + (-25 * sign_pos_neg), flagy2 - 5)],
                        (204, 204, 0),
                    )
