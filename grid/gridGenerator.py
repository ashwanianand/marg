...
def random_grid_with_buechi_region_meanpayoff_region(size, wall_density=0.3, region_ratio=0.2, min_distance=5, max_distance=10, ignore_attributes=True, probabilistic_ratio=0.2):
    ...
    for (row, column) in iterate_over_cells(size):
        ...
        grid[row][column]['weight'] = random_cell_weight # contains mean payoff weight and then other parity objectives. the weights are split later in the conversion to games
    ...
    if not ignore_attributes:
        grid[robot_row][robot_column]['attributes'] = grid[robot_row][robot_column]['attributes'] + ' robot'
    print("sending new grid to client")
...
def random_objective(size, region_size):
    rows = cols = size // 2
    """Creates the grid and places two regions of 1s and 2s"""
    grid = [[0] * cols for _ in range(rows)]
    
    # Place the first region of 1s
    _ = place_region(grid, region_size, 2)

    objective = {}

    for row, col in iterate_over_cells(size//2):
        objective[(row, col)] = grid[row // 2][col // 2]
...
def modify_grid_objectives(grid, objectives):
    # objectives is a dictionary with key as index and value as the objective which maps state to the value of the objective
    for index, objective in objectives.items():
        for row, col in iterate_over_cells(len(grid) // 2):
            grid[row][col]['weight'][index] = objective[(row, col)]